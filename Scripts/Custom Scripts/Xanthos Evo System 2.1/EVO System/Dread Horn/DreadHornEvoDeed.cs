...
{
    public abstract class DreadHornEvoDeed : Item
	{
		public abstract IEvoCreature GetEvoCreature();

		public DreadHornEvoDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
		...
		public DreadHornEvoDeed( Serial serial ) : base ( serial )
		{
		}
		...
		public void Use( Mobile from )
		{
			...
			if ( null != evo )
			{
                BaseEvoSpec spec = DreadHornEvoBaseEgg.GetEvoSpec ( evo );
				...
				if ( null != spec && null != spec.Stages && !creature.Deleted )
				{
					...
					else
					{
						...
						Delete();
						from.SendMessage( "You are now master of a dread horn." );
					}
				}
			}
		}
